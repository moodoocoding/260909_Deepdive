...
const missionItems: MissionItem[] = [
  { id: "laptop", name: "노트북", answer: "computer", reason: "키보드로 입력받고, 프로그램으로 처리해 화면에 결과를 보여 줍니다.", icon: Laptop, tone: "bg-sky-50 text-sky-700" },
  { id: "book", name: "종이책", answer: "not-computer", reason: "정보를 담고 있지만, 입력을 받아 스스로 처리하거나 출력하지는 않습니다.", icon: BookOpen, tone: "bg-amber-50 text-amber-700" },
  { id: "robot-vacuum", name: "로봇청소기", answer: "computer", reason: "센서로 주변을 입력받고, 프로그램으로 경로를 처리해 움직임을 출력합니다.", icon: Bot, tone: "bg-violet-50 text-violet-700" },
  { id: "scissors", name: "가위", answer: "not-computer", reason: "사람의 힘으로 움직이는 도구이며 정보를 처리하는 프로그램이 없습니다.", icon: Scissors, tone: "bg-rose-50 text-rose-700" },
  { id: "smartphone", name: "스마트폰", answer: "computer", reason: "터치와 센서로 입력받고, 앱으로 처리해 화면·소리 등으로 출력합니다.", icon: Smartphone, tone: "bg-cyan-50 text-cyan-700" },
  { id: "umbrella", name: "일반 우산", answer: "not-computer", reason: "비를 막는 도구이지만 정보를 입력·처리·출력하는 구조는 없습니다.", icon: Umbrella, tone: "bg-indigo-50 text-indigo-700" },
  { id: "traffic-light", name: "디지털 신호등", answer: "computer", reason: "신호나 센서 정보를 받아 프로그램에 따라 불빛을 제어합니다.", icon: Lightbulb, tone: "bg-emerald-50 text-emerald-700" },
  { id: "bike", name: "일반 자전거", answer: "not-computer", reason: "사람의 힘을 전달하는 기계 장치이며 프로그램으로 정보를 처리하지 않습니다.", icon: Bike, tone: "bg-orange-50 text-orange-700" },
];
...
function Dashboard({ onStart, completed }: { onStart: () => void; completed: boolean }) {
  return (
    <div className="mx-auto w-full max-w-[1180px] px-5 pb-12 pt-8 sm:px-8 lg:px-10 lg:pt-12">
      ...
      <section className="mt-10 grid gap-5 lg:grid-cols-[1.45fr_0.75fr]">
        <article className="relative overflow-hidden rounded-[28px] bg-[#113f4b] p-7 text-white shadow-[0_18px_50px_rgba(15,57,68,0.18)] sm:p-9">
          ...
          <div className="relative z-10 max-w-xl">
            ...
            <div className="mt-7 flex flex-wrap items-center gap-4">
              <Button size="lg" className="h-12 rounded-xl bg-[#f5c85d] px-5 font-bold text-[#183c42] shadow-lg shadow-slate-950/20 hover:bg-[#ffda78]" onClick={onStart}>{completed ? "다시 도전하기" : "미션 시작하기"} <ArrowRight className="size-4" /></Button>
        ...
        <article className="flex flex-col rounded-[28px] border border-slate-200 bg-white p-7 shadow-sm">
          <div className="flex items-start justify-between">
            <div><p className="text-sm font-bold text-slate-500">나의 연수 진행률</p><p className="font-display mt-2 text-4xl font-bold tracking-tight text-slate-950">{completed ? 100 : 0}%</p></div>
          ...
          <Progress value={completed ? 100 : 0} className="mt-7 h-2.5 bg-slate-100 [&_[data-slot=progress-indicator]]:bg-[#27a69c]" />
          <div className="mt-auto grid grid-cols-2 gap-3 pt-7">
            <div className="rounded-2xl bg-slate-50 p-4"><p className="text-2xl font-bold text-slate-900">{completed ? 1 : 0}</p><p className="mt-1 text-xs font-semibold text-slate-500">완료한 미션</p></div>
            <div className="rounded-2xl bg-slate-50 p-4"><p className="text-2xl font-bold text-slate-900">1</p><p className="mt-1 text-xs font-semibold text-slate-500">진행 가능</p></div>
          </div>
        </article>
      </section>
      ...
      <section className="mt-11">
        <div className="mb-5 flex items-end justify-between">
          ...
          <span className="hidden text-sm font-semibold text-slate-400 sm:block">1개의 미션</span>
        </div>
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          <button onClick={onStart} className="group rounded-[24px] border border-slate-200 bg-white p-6 text-left shadow-sm transition-all hover:-translate-y-1 hover:border-[#70c8c0] hover:shadow-xl hover:shadow-slate-200/60 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-600/20">
            <div className="flex items-start justify-between">
              ...
              <span className={cn("rounded-full px-3 py-1.5 text-xs font-bold", completed ? "bg-emerald-50 text-emerald-700" : "bg-[#eaf8f5] text-[#0d7771]")}>{completed ? "완료" : "진행 가능"}</span>
            </div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-500">입력–처리–출력의 관점으로 주변 사물을 분류합니다.</p>
            <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-5 text-sm">
              ...
              <span className="flex items-center gap-1 font-bold text-[#0d7771]">{completed ? "다시 하기" : "시작하기"} <ChevronRight className="size-4 transition-transform group-hover:translate-x-1" /></span>
            </div>
          </button>

          <div className="flex min-h-[278px] items-center justify-center rounded-[24px] border border-dashed border-slate-300 bg-white/50 p-7 text-center">
            <div><div className="mx-auto grid size-11 place-items-center rounded-2xl bg-slate-100 text-slate-400"><LockKeyhole className="size-5" /></div><p className="mt-4 text-sm font-bold text-slate-600">다음 미션을 준비하고 있어요</p><p className="mt-1 text-xs leading-relaxed text-slate-400">새로운 활동이 이곳에 차례로 추가됩니다.</p></div>
          </div>
        </div>
      </section>
    </div>
...
function Mission({ onBack, onComplete }: { onBack: () => void; onComplete: () => void }) {
  ...
  const choose = (id: string, category: Category) => { if (!submitted) setAnswers((current) => ({ ...current, [id]: category })); };
    ...
    const register = modelContext.registerTool(
      {
        ...
        title: "컴퓨터 분류 미션 완료",
        description: "8개 사물을 컴퓨터 또는 컴퓨터 아님으로 한 번에 분류하고 결과 화면을 엽니다.",
        inputSchema: {
          type: "object",
          properties: {
            answers: {
        ...
        execute(input: unknown) {
          ...
          if (!candidate?.answers || Object.keys(candidate.answers).length !== missionItems.length) throw new Error("8개 사물의 분류가 모두 필요합니다.");
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] px-5 pb-12 pt-7 sm:px-8 lg:px-10 lg:pt-9">
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-start">
          <div>
            ...
            <p className="mt-3 text-[0.98rem] leading-relaxed text-slate-500">각 사물이 컴퓨터인지 생각하고 알맞은 쪽을 선택하세요.</p>
          </div>
        ...
        <div className="mt-7 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3.5">
          <Lightbulb className="mt-0.5 size-5 shrink-0 text-amber-600" aria-hidden="true" />
          <p className="text-sm leading-relaxed text-amber-900"><strong>생각 열쇠</strong> · 전기를 사용하는지만 보지 마세요. 정보를 <strong>입력</strong>받아 프로그램으로 <strong>처리</strong>하고 결과를 <strong>출력</strong>하는지 살펴보세요.</p>
        </div>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {missionItems.map((item) => {
            ...
            return (
              <article key={item.id} className={cn("rounded-[22px] border bg-white p-4 transition-all", selected ? "border-[#66bdb5] shadow-md shadow-teal-100/50" : "border-slate-200")}>
                <div className={cn("grid aspect-[1.55] place-items-center rounded-2xl", item.tone)}><Icon className="size-14" strokeWidth={1.55} aria-hidden="true" /></div>
                <h2 className="mt-4 text-center text-base font-bold text-slate-900">{item.name}</h2>
                <div className="mt-3 grid grid-cols-2 gap-2" role="group" aria-label={`${item.name} 분류`}>
                  <button onClick={() => choose(item.id, "computer")} aria-pressed={selected === "computer"} className={cn("rounded-xl border px-2 py-2.5 text-xs font-bold transition-all", selected === "computer" ? "border-[#0d7771] bg-[#0d7771] text-white" : "border-slate-200 text-slate-500 hover:border-[#70c8c0] hover:bg-[#eff9f7]")}>컴퓨터</button>
                  <button onClick={() => choose(item.id, "not-computer")} aria-pressed={selected === "not-computer"} className={cn("rounded-xl border px-2 py-2.5 text-xs font-bold transition-all", selected === "not-computer" ? "border-slate-700 bg-slate-700 text-white" : "border-slate-200 text-slate-500 hover:border-slate-400 hover:bg-slate-50")}>컴퓨터 아님</button>
                </div>
              </article>
            );
          })}
        ...
        <div className="mt-8 flex flex-col gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-slate-500">모든 사물을 분류하면 결과를 확인할 수 있어요.</p>
...
export default function HomePage() {
  ...
  const [screen, setScreen] = useState<"dashboard" | "mission">("dashboard");
  const [completed, setCompleted] = useState(false);
  const startMission = () => { setScreen("mission"); window.scrollTo({ top: 0, behavior: "smooth" }); };
    ...
    <main className="min-h-screen bg-[#f6f8f9] text-slate-900">
      ...
      <div className="min-h-screen lg:pl-[268px]">
        <header className="sticky top-0 z-30 flex h-[73px] items-center justify-between border-b border-slate-200 bg-white/90 px-5 backdrop-blur-md sm:px-8 lg:px-10">
          ...
          <div className="hidden items-center gap-2 text-sm font-semibold text-slate-400 lg:flex"><Home className="size-4" /> 대시보드{screen === "mission" && <><ChevronRight className="size-4" /><span className="text-slate-700">컴퓨터를 찾아라!</span></>}</div>
          <div className="ml-auto flex items-center gap-3">{completed && <span className="hidden items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 sm:flex"><CheckCircle2 className="size-3.5" /> 미션 1 완료</span>}<div className="grid size-9 place-items-center rounded-full bg-[#e9f5f3] text-sm font-bold text-[#0d615e]">김</div></div>
        </header>
        {screen === "dashboard" ? <Dashboard onStart={startMission} completed={completed} /> : <Mission onBack={() => setScreen("dashboard")} onComplete={() => { setCompleted(true); setScreen("dashboard"); window.scrollTo({ top: 0, behavior: "smooth" }); }} />}
      </div>
    </main>
  );
}
